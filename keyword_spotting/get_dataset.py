...
def convert_dataset(item):
  ...
  label = item['label']
  return audio, label
def convert_dataset_trecx(item):
  """Puts the mnist dataset in the format endpoint layer of t-recx expects, (features, labels), labels."""
  audio = item['audio']
  label = item['label']
...
def get_training_data(Flags, get_waves=False, val_cal_subset=False):
  ...
  if get_waves:
    ...
    if Flags.isTrecx or Flags.model_architecture=='ds_cnn_sdn':#if using t-recx models or using sdn use custom data generator 
      ds_train = ds_train.map(convert_dataset_trecx)
      ds_test = ds_test.map(convert_dataset_trecx)
      ds_val = ds_val.map(convert_dataset_trecx)
    else:
  ...
  ds_val = ds_val.batch(Flags.batch_size, drop_remainder=True)
  
  # ds_train = ds_train.padded_batch(Flags.batch_size, pad)
  # ds_test = ds_test.padded_batch(Flags.batch_size)  
  # ds_val = ds_val.padded_batch(Flags.batch_size)
