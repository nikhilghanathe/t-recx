...
def main(argv):
  ...
  model = models.mobilenet_v1_no_mv()
  ...
  with open('train_data.npy', 'wb') as fp:
    np.save(fp, train_data)
  with open('train_labels.npy', 'wb') as fp:
    np.save(fp, train_labels)

  print(train_generator.class_indices)

  ss

  model = train_epochs(model, train_generator, val_generator, 20, 0.001, model_name)
  model = train_epochs(model, train_generator, val_generator, 10, 0.0005, model_name)
  model = train_epochs(model, train_generator, val_generator, 20, 0.00025, model_name)
