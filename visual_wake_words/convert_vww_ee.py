...
def main(argv):
    ...
    model_base = tf.keras.models.load_model(argv[1])
    model_base.summary()
    cnt =0
    for layer in model_base.layers:
        ...
        if layer.name=='dense_1':
            ef_num = cnt
        cnt+=1
    model = tf.keras.models.Model(inputs=model_base.inputs[0], outputs=[model_base.layers[ee_num].output, model_base.layers[ef_num].output] )
